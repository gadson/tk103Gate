...
from app.settings import GOOGLE_API_KEY, FROM_EMAIL, RC_USERNAME, RC_PASSWORD, RC_DOMAIN
        ...
        if password == password2:
            ...
            if str(userr) == '[]':
                ...
                if user is not None:
                    ...
                    email_user = EmailMessage(subject='Регистрация в системе Авторадар', body='Поздравляем! Вы зарегистрировались в системе Авторадар. Введите ID ключ в программу Alarm Service в мультимедийной системе Вашего автомобиля. Ваш логин: '+username+' , пароль: '+ password, from_email='info@antec-it.ru', to=[email])
                    try:
                        email_user.send()
                    except:
    ...
    if request.method == 'POST' and request.user.is_authenticated:
       ...
       if Counts < profile.maxobject:
           ...
           email_user = EmailMessage(subject='ID ключ дл Вашего Авторадара',
                                     ...
                                     from_email='info@antec-it.ru', to=[user.email])
           try:
               email_user.send()
           except:
